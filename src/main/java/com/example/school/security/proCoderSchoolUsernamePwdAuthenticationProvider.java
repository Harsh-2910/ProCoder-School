...
@Component
@Profile("prod")
public class proCoderSchoolUsernamePwdAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        if( null!=person && person.getPersonId()>0 && passwordEncoder.matches(pwd,person.getPwd())){
            return new UsernamePasswordAuthenticationToken(email, person.getPwd(), getGrantedAuthorities(person.getRole()));
        }
        else {
